...
## Any configuration?
There's no configuration UI, since it's supposed to be fire and forget type macro, but you can set-up it once by adding special parameters. Add them into first line between `({})` brackets.
...
So zone ability itself should disappear when macro is on your action bars and your action bars should show ability cooldown/tooltip properly.
Unfortunately there are currently bug with macros unable to find correct copy of "Unraveling Sands" among several existing in game, so it often doesn't work - in such cases you still have to park zone ability somewhere on bars yourself.
